<?php session_start(); ?>
<!Doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <title>Inicio</title>
    <link rel="stylesheet" href="css/bootstrap.min.css">
    <link rel="stylesheet" href="http://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.6.3/css/font-awesome.min.css">
      <link href="css/home2.css" rel="stylesheet">

      <style>
      .bd-placeholder-img {
        font-size: 1.125rem;
        text-anchor: middle;
      }
      @media (min-width: 768px) {
        .bd-placeholder-img-lg {
          font-size: 3.5rem;
        }
      }
      body {overflow-x:hidden;}
      body::-webkit-scrollbar{
        width: 10px;
        background: #00001a;
      }
      body::-webkit-scrollbar-thumb{
        background: rgba(255, 255, 255, .3);
        border-radius: 10px;
        border-right: 2px solid #00001a;
      }
      </style>
  </head>

  <body>
<!-- Menu de Navegación -->
      <nav class="navbar navbar-expand-lg fixed-top navbar-dark" style="background-color:#00001a; ">
        <a class="navbar-brand" href="#">
         <img src="res/logo.png" width="50" height="50" alt="">
         Strongmen Academy
       </a>
       <button class="navbar-toggler" type="button" data-toggle="" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
         <span class="navbar-toggler-icon"></span>
       </button>
          <div class="collapse navbar-collapse" id="navbarSupportedContent">
           <ul class="navbar-nav mr-auto">
             <li class="nav-item active">
               <a class="nav-link" href="home2.html">Inicio  <span class="sr-only">(current)</span></a>
             </li>
             <li class="nav-item">
               <a class="nav-link" href="asesorias.html">Asesorias</a>
             </li>
             <li class="nav-item">
               <a class="nav-link" href="aulas.html">Aulas</a>
             </li>
           </ul>
           <ul class="navbar-nav navbar-right ">
             <li class="nav-item ">
               <a class="nav-link" href="registro.php">Registro</a>
             </li>
             <li class="nav-item ">
               <a class="nav-link" href="login.php">Login</a>
             </li>
           </ul>
         </div>
      </nav>
<!-- Fin Menú -->
     <!-- page-header-portada -->
  <div class="page-header">
      <div class="container">
          <div class="row">
              <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                  <div class="page-caption">
                      <h1 class="page-title">Stay Strong</h1>
                      <hr color= "ffffff">
                      <h4 class="page-description" strong > Centro Deportivo, dedicado a la fuerza,el alto rendimiento, la rehabilitación deportiva y la prescripción de ejercicio de manera adecuada </h4>
                  </div>
              </div>
          </div>
      </div>
  </div>
  <!-- /.page-header-->

  <!-- Sección -->
          <section>
            <br>
              <div class="container marketing col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 ">
                <center><H1 style="color:#424242"> Servicios </h1></center>
            <div class="row">
            <div class="col-lg-6">
              <a href="asesorias.html"> <img src="res/foto1.jpg" class="bd-placeholder-img rounded-circle img-fluid m-x-auto" width="300" height="300" preserveAspectRatio="xMidYMid slice" focusable="false" role="img" aria-label="Placeholder: 140x140"></a>
              <br></br>
            <center>  <h2 style="color:#424242">Asesorias</h2> </center>
              <p style="color:#424242">Asesorate con los mejores, sube y baja archivos para mejorar tus estadisticas y actualizar tu información, tambien peudes entablar una videollamada y con tu
                entrenador par aclarar dudas.</p>
              <p>
            </div>
            <div class="col-lg-6">
              <a href="aulas.html"> <img src="res/foto2.jpg" class="bd-placeholder-img rounded-circle img-fluid m-x-auto" width="300" height="300" preserveAspectRatio="xMidYMid slice" focusable="false" role="img" aria-label="Placeholder: 140x140"></a>
              <br></br>
              <center> <h2 style="color:#424242">Aulas</h2></center>
              <p style="color:#424242">Revisa los nuevos cursos que tenemos para ti, cursos de pesas hasta isometricos, elige lo que a ti mas te convenga de entre todo nuestro repertorio además ve los videos cuando y donde quieras.</p>
            </div>
            </div>
              <br></br>
            <hr color= "#F7F6F6">
            <br> </br>
              <!-- 2a Sección -->

          <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 ">
            <div class="container-fluid text-center">
              <h3 style="color:#424242">¿Como te ayudamos?</h3>
              <br>
              <div class="row">
                <div class = "col-sm-4 col-md-4">
                  <div class = "img-thumbnail">
                   <img src = "res/publicidad/foto2.png" class="img-fluid m-x-auto zoom " width="300" height="300">
                   <hr color= "#F7F6F6">
                   <h5><p>Entrena tu mente. <span class="text-muted">y tu cuerpo tambien.</span></p></h5>
                   <hr color= "#F7F6F6">
                   <p>Pacticar deportes, ir al gimnasio, salir a correr o simplemente agregar un poco de movimiento a tu vida, no solo reflejará resultados a nivel físico, sino que también, habrá númerosos beneficios a nivel psicológico/mental que complementerán tu estado de salud general. Recuerda que, lo que estés haciendo hoy, determinará bastante como vivas mañana. </p>
                      </div>
                 </div>

                 <div class = "col-sm-4 col-md-4">
                  <div class = "img-thumbnail">
                   <img src = "res/publicidad/foto3.jpg" class="img-fluid m-x-auto zoom " width="300" height="300">
                   <hr color= "#F7F6F6">
                   <h5><p>No hay edad para<span class="text-muted">empezar a entrenar.</span></p></h5>
                   <hr color= "#F7F6F6">
                   <p>Cada vez se conoce más sobre los beneficios que el entrenamiento con pesas o lastres puede tener sobre la salud y el rendimiento desde muy tempranas edades. Que no te sigan contando o engañando con mitos sin fundamento; los niños/as DEBEN hacer ejercicio y PUEDEN ir al gimnasio. En manos de un verdadero profesional, el riesgo es el mismo al de jugar fútbol o practicar taekwondo.</p>
                      </div>
                 </div>

                 <div class = "col-sm-4 col-md-4">
                  <div class = "img-thumbnail">
                   <img src = "res/publicidad/equipo.jpg" class="img-fluid m-x-auto zoom " width="300" height="300">
                   <hr color= "#F7F6F6">
                   <h5><p>Excelente ambiente<span class="text-muted"> y equipo.</span></p></h5>
                   <hr color= "#F7F6F6">
                   <p>Contamos con equipo de calidad para otorgarte una mejor facilidad y .....</p>
                      </div>
                 </div>

          </section>
  <!-- Sección -->

  <br></br>
   <hr color= "#F7F6F6">
   <br></br>
   <!--Footer-->

  <footer id="myFooter">
   <div class="container">
       <div class="row">
           <div class="col-sm-3">
               <h2 class="logo">Strongmen Academy</a></h2>
           </div>
           <div class="col-sm-2">
               <h5>Contactanos</h5>
               <ul>
                   <li><a href="#"><i class="fa fa-phone"> Arturo: </i></a></li>
                   <li><a href="#">5565133884</a></li>
                   <li><a href="#"><i class="fa fa-phone"> Emmanuel: </i> </a></li>
                   <li><a href="#">7774226470</a></li>
               </ul>
           </div>
           <div class="col-sm-2">
               <h5>Direccción</h5>
               <ul>
                   <li><a href="#">Cuijingo,Estado de México</a></li>
               </ul>
           </div>
           <div class="col-sm-2">
               <h5>Acceso Rapido</h5>
               <ul>
                   <li><a href="asesorias.html"><i class="fa fa-arrow-right"> Asesorias</i></a></li>
                   <li><a href="aulas.html"><i class="fa fa-arrow-right"></i> Aulas</a></li>
               </ul>
           </div>
           <div class="col-sm-3">
               <div class="social-networks">
                   <a href="https://www.instagram.com/?hl=es-la" class="instagram"><i class="fa fa-instagram"></i></a>
                   <a href="https://www.facebook.com/" class="facebook"><i class="fa fa-facebook"></i></a>
                   <a href="mailto:strongmenlifters@gmail.com" class="google"><i class="fa fa-envelope"></i></a>
               </div>
               <button type="button" class="btn btn-default"><a href="registro.php">Registrate</a></button>
           </div>
       </div>
   </div>
   <div class="footer-copyright">
       <p>© 2019 Strongmen Academy. Todos los derechos Reservados </p>
   </div>
  </footer>


      <!--Fin Footer-->

    <script src="js/jquery-3.3.1.min.js"></script>
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.12.4/jquery.min.js"></script>
    <script src="js/popper.min.js"></script>
    <script src="js/bootstrap.min.js"></script>
  </body>

  </html>
